...
void processPacket(gdl90Data_t *packet) {
    switch (packet->id) {
        ...
        case GDL90_OWNSHIP_REPORT:
            setOwnshipPosition(&packet->positionReport, lastHeartbeat.gpsPosValid);
            break;
        ...
        case GDL90_FOREFLIGHT_ID:
            snprintf(gdl90StatusText, sizeof(gdl90StatusText), "Foreflight %s/%s %s",
                     packet->foreflightId.name, packet->foreflightId.longName,
                     packet->foreflightId.wgs84 ? "WGS84" : "MSL");
            break;
...
static void gCallback(const gdlDataPacket_t *packet) {
    gdl90Data_t data;
